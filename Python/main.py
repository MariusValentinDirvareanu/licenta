...
def KeyExpansionCore(in1, i):

	# Rotatie stanga
	in1 = (in1 >> 8) | ((in1 & 0xff) << 24)
...
def expandareCheie(inputKey, expKeys):
	expKeys=inputKey
	bytesGenerated = 16
	rconIteration = 1
	temp=[]
...
def ShiftRows(state):
	...
	state[5], state[9]=state[9], state[5]
	state[6], state[14]=state[14], state[6]
	state[7], state[15]=state[15], state[7]
	state[9], state[13]=state[13], state[9]
	state[11], state[15]=state[15], state[1]
...
def main():
	mesaj= "Acesta este mesajul meu."
	cheie = "u43x2l6gjng24edf"
	print(mesaj)
	originalLen = len(mesaj)
	print(originalLen)
	lenOfPaddedMessage = originalLen
	if (lenOfPaddedMessage % 16 != 0):
		lenOfPaddedMessage = (lenOfPaddedMessage // 16 + 1) * 16

	print(lenOfPaddedMessage)
	paddedMessage = mesaj
	i=originalLen
	while (i < lenOfPaddedMessage):
		paddedMessage+'0'
		i=i+1

	i=0
	while (i < lenOfPaddedMessage):
		criptareMesaj(paddedMessage + str(i), cheie)
		i = i+16

	print("\n Mesaj criptat:\n")

	for i in range(0, lenOfPaddedMessage):
